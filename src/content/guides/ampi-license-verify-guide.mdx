...
The only figure on this page with real weight is the independent attorney at $1,500 to $5,000, since AMPI membership is a voluntary signal rather than a licence and carries no indemnity. Use the table below as underwriting stress inputs, and verify current bank, insurer and municipal rules before closing.
...
Chapter dues and verification procedures vary by state; use the bands above for budgeting and confirm the current process with the local AMPI chapter directly.

---

## How do foreign buyers complete this purchase legally?

Foreign ownership in Mexico is legally settled and procedurally routine, which is precisely why the agent question matters. The structure — a fideicomiso inside the restricted zone, direct title outside it, remote closing by power of attorney where needed — has been working for decades and is not where transactions fail. They fail on execution: unverified wire instructions, closing costs that arrive 5-10% above the price nobody budgeted for, and short-term-rental assumptions that the building's rules do not support. The balance below sets out what the legal framework gives a foreign buyer and what it leaves exposed.
