...
Registration is worth roughly $2,100 a year on a typical Playa rental and that is the clearest reason to do it early. Platforms withhold ISR against the RFC: an unregistered foreign host sits in the higher band near 20% of gross, while a registered one sits near 10% and can elect taxation on net income after management, HOA, predial and depreciation. On $21,000 of gross that gap is $2,100, against setup costing $300 to $800 through a contador. The election is made at registration and is not retroactive within the calendar year.
...
You do not need residency to hold an RFC. SAT issues one to temporary residents with and without income authorisation, to permanent residents, and to non-resident property owners registering for passive rental income — the last being the category most foreign buyers fall into. What differs between them is not eligibility but how much of the tax system you can then access.
...
Yes, you need both, and in that order: CURP is a population ID from RENAPO and it is a prerequisite for RFC registration, which is SAT's 13-character tax identifier. Trying to register for an RFC without a CURP is the single most common reason a first attempt fails, and it is entirely avoidable.
...
Document preparation is the single factor most responsible for failed or incomplete SAT appointments. Arrive with complete documentation or reschedule rather than attempt registration with incomplete materials. Expect passport, CURP where you hold one, proof of a Mexican address such as a utility bill or the escritura, and a comprobante de domicilio no older than three months; a contador at $300 to $800 assembles the file and books the appointment. SAT slots are scarce in Playa and Cancún, so a rescheduled appointment commonly costs 3 to 6 weeks, which is why the document check matters more than the queue.
...
The connection between your RFC and CFDI receipts is the reason most tax advisors recommend getting RFC before or at closing, not years later. This matters specifically for future capital gains calculations. Without an RFC on file, a contractor or vendor cannot issue a CFDI in your name, so the payment sits outside the cost basis the notario uses at sale. On a $70,000 undocumented renovation and a later sale at $420,000, that omission can add roughly $24,500 of ISR withholding at the 35% net method — against $300 to $800 to register.

RFC is what makes a CFDI issuable to you, which is what makes an expense count toward your Mexican cost basis. Without one, renovation and improvement payments sit outside the basis the notario uses at sale, so a $70,000 undocumented spend on a property later sold at $420,000 can add roughly $24,500 of ISR withholding at the 35% net method. The registration that prevents that costs $300 to $800 and takes days; there is no route to reconstruct the invoices years later, because a CFDI must be issued at the time of payment by the party being paid.
...
## RFC registration inside a fideicomiso

The bank's fiscal registration is not yours. As beneficiary you need your own RFC to report rental income, claim deductions against a capital gain, and receive CFDIs attributed to you — the trust may have separate trust-level obligations, but it does not discharge your personal ones. Owners who assume the bank handles it discover otherwise at sale.
...
This is the whole financial case for registering. Without an RFC a non-resident pays 25% of gross rental income with no deductions at all; with an RFC you can elect to be taxed on net income after management, HOA, depreciation and interest. On a property with real operating costs, that election is usually worth several thousand dollars a year.
...
## Where you register: SAT office by market

SAT's capacity for foreign registrants varies sharply by market, and it is worth choosing your appointment accordingly. Cancún and Playa del Carmen process high foreign volume with some English capability; smaller Baja and Pacific offices see far fewer non-resident registrations and can be slower for it. Book the appointment before you assemble the documents — the queue is the constraint.
...
## Common RFC registration mistakes

Four errors account for most failed registrations, and the costliest is timing: leaving RFC registration until a sale is imminent, when years of CFDI receipts were never attributed to a tax ID that existed. The others are procedural — assuming the trustee bank handles it, name mismatches across passport, CURP and RFC, and proof of address older than three months.
...
## RFC, FBAR, and US foreign asset reporting: how
...
Most of this belongs before closing, not after. Check whether you already hold a CURP, assemble the document package, and pre-register online a week or two before the SAT appointment — the visit itself is short when the paperwork is right and a wasted trip when it is not.
