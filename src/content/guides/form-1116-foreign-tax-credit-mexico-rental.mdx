...
Fixing the problem upstream beats optimising the credit downstream. The trap in this example is the 25% withholding struck on gross receipts: $21,250 of Mexican tax against a US taxable rental result of only $25,000, which caps the current-year credit at $5,000 and strands $16,250 in carryforward. An owner registered with an RFC and electing to be taxed on net rental income in Mexico would pay ISR on a base close to the Schedule E figure rather than on gross, and the two systems would broadly align. That election is made at registration and is not retroactive.
...
Running both numbers on the worked example shows the decision is not automatic. Crediting produces $5,000 of current-year benefit plus $16,250 of carryforward that may never be used. Deducting the full $21,250 against a 24% marginal rate produces about $5,100 of certain benefit this year and nothing to track. Where the limitation blocks most of a credit and there is no realistic passive-basket capacity in the carryforward window, the deduction can be the better outcome — which is why this belongs in a CPA's projection rather than in a rule of thumb.
...
Before preparing the US return, organize the Mexico rental file around the questions a preparer actually has to answer, because a credit that cannot be substantiated is a credit that does not get claimed. Form 1116 needs three things reconciled: gross rental income as reported to Mexico, the ISR actually paid there, and the expense set claimed on Schedule E. Where those three disagree — platform gross versus manager net remittances is the usual culprit — the preparer either spends billable hours reconciling them or defaults to the conservative position, and the conservative position leaves foreign tax credit on the table. Assemble the file below once and it carries forward every year.
