...
Modelling from the annual average is the mistake that empties reserves. Playa Centro runs 68% to 78% across the year but troughs at 50% to 62% in September, and the $300 to $500 HOA, predial and trust fee run unchanged through it. An owner who underwrote 73% and holds no cash faces a two-month deficit in their first low season. Build the model month by month from the corridor's actual calendar, then hold three months of fixed costs — roughly $1,500 to $2,500 on a typical 1BR — before the first booking.
...
### How to read nightly-rate models on this site

Two kinds of number appear across these pages and they are not the same thing. The corridor bands — 5.5% to 7% gross, and 4.3% to 5.2% net in prime Playa colonias — are full-year averages that already carry weak months, imperfect operation and real vacancy. The worked models on individual project and area pages multiply a nightly rate by an occupancy percentage across a 365-night calendar, and several of them imply a gross above 12%.

Those models are projections under good conditions, not reported results. They describe a stabilised listing with review history, a professional operator and a strong year, and each such page now says so on the model itself. Where a project page shows both an optimistic and a conservative case, the conservative one is the planning number. Where a seller or developer supplies a model, ask instead for twelve months of actual operating statements from a comparable unit in the same building.
...
Ranking the threats by expected cost puts HOA drift first, ahead of the ones buyers worry about. A fee rising 6% a year from $400 takes $4,800 of annual cost to $8,600 within a decade, roughly 120 basis points of yield on a $300,000 unit, and no owner votes it down alone. Identical-unit competition is second, worth up to 180 basis points between a well-chosen colonia and an oversupplied tower. Permit enforcement is third — severe when it lands, but avoidable with registration. Hurricane damage is fourth, and insurable.
...
A yield figure is worth nothing if the unit cannot legally be rented nightly, and that is settled by four documents rather than by the listing. Confirm all four before the deposit clears, in writing:
...
Tulum trades as one destination and underwrites as two. Established Aldea Zama stock and the newer inland regions produce net yields two to three points apart on similar entry prices, because infrastructure and supply differ rather than demand:
...
Los Cabos: Branded corridor inventory trades at premium, gross can match RM but HOA $400–1,200/month compresses net toward 3–4%. Appreciation and USD buyer depth often carry the thesis. The Pacific pattern in one line: gross yields comparable to the Caribbean, net yields a point lower because HOA and management both run higher, and a milder hurricane profile as partial compensation.
...
Cash flow and capital growth pull toward different buildings in Mexico, and buyers who refuse to choose usually end up with neither. Three profiles weight them differently, and each accepts a different net yield floor:
...
The sources below sit in a deliberate hierarchy. A seller's trailing-twelve P&L cross-checked against their CFDI rental invoices and ISR filings beats a property manager's quote, because the manager is quoting to win the mandate; a manager's quote beats a scraping tool such as AirDNA, which cannot see net-of-fee reality or which listings sat empty; and the scraping tool beats AMPI broker comparables, which are anchored on asking ADRs rather than realised ones. Nothing on this list outranks the model you build yourself from the actual bank statements.
...
A strong gross yield on a unit with a title or governance problem is a discount for risk, not a bargain. Three findings should end the conversation whatever the pro forma says: The three that end conversations rather than adjust prices: a rental ban in the regime, an unresolved title or ejido question, and a special assessment already voted but unpaid. Each is a discount for risk that a yield figure cannot express.
...
The management fee headline is 25% to 35%, but the clauses underneath it decide what you actually keep. Read these three before signing with any manager: The three worth reading twice: whether the fee is calculated before or after platform fees, whether maintenance markups are capped, and who holds the municipal rental registration. The last one determines whether you can change manager at all.
...
The label "Playa del Carmen" covers buildings whose net yields sit as much as 300 basis points apart, so postcode-level data is close to useless for underwriting. Two buildings in the same colonia can differ by more than two different markets do.
...
A seller's trailing twelve months of actual operating results beats any forward pro forma, because one is a record and the other is an argument. The exception is a unit you intend to renovate, where the trailing number describes a property that will no longer exist.
...
A national average vacation-rental yield for Mexico is not a useful number. The spread between Playa Centro and Tulum Region 15 is wider than the spread between most countries, which means any figure quoted at country level is averaging away the only variable that matters.
...
Leverage turns a modest yield negative faster in Mexico than most buyers expect. A 4.5% net unlevered return against a 9% mortgage at 65% LTV produces negative monthly cash flow, and that combination is common rather than exotic in this market.
...
The colonia net bands in this guide are what honest underwriting produces on typical stock, so a quote far above them is a claim that needs evidence rather than a better opportunity. If a broker quote exceeds the relevant band by 150+ bps, request line-item pro forma or walk. Outlier claims require outlier proof.

---

## Yield guide summary

Mexican coastal short-term rental property nets 3% to 5% in the liquid markets once HOA, management, lodging tax and vacancy are in the model. That is the number to plan around: use the colonia tables above as a sanity check on any quote, use colonia tables here as sanity check, not ceiling promise.
