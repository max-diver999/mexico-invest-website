...
A project in delivery has traded its biggest discount for its biggest risk reduction. Construction risk is largely behind you and prices sit near market rather than at launch, which changes what diligence is for: you are now verifying finished quality and the real HOA figure rather than a developer's capacity to build at all.
...
Three formats from about $225,000, and the lock-off two-bedroom is the one that changes the arithmetic: it can be rented as two units or one, which raises gross in high season and gives a fallback in the trough. The one-bedroom at $280,000–360,000 remains the workhorse for a single-unit buyer.
...
Region 15's short-term rental supply grew roughly fivefold between 2020 and 2026, and that pressure reaches Holistika indirectly: a budget-constrained guest comparing an $80 Region 15 studio against a $150 wellness unit is a guest the differentiation has to win. It usually does — but the discount gap sets a ceiling on how far ADR can rise.
...
The wellness positioning buys ADR rather than occupancy: peak rates of $160–220 on a one-bedroom against generic Tulum inventory, at occupancy in the 65–75% band that is normal for the market. That is the right trade in an oversupplied town — rate holds up better than occupancy when competitors discount.
...
The amenities here are not a lifestyle brochure, they are the reason the ADR premium exists: a cenote or cenote-adjacent pool that supplies the primary booking image, jungle yoga platforms, dedicated sound and meditation spaces, and on-site dining that removes the transport problem a jungle location otherwise creates. Remove any of them and the unit prices like Region 15.
...
Brand recognition is not a permit. The two items that matter most on this specific project are municipal construction permits for the delivering phase and SEMARNAT environmental authorisation — cenote-adjacent land in Tulum has drawn stop orders since 2024, and an environmental irregularity attaches to the property rather than to the developer.
...
Holistika's differentiated positioning is what keeps most of these risks at low or medium rather than high, and that is worth understanding as a dependency rather than a feature. Region 15 oversupply spills into any building that competes on generic amenity; a wellness identity with a cenote-adjacent location competes on something the surrounding inventory cannot copy quickly. If that identity weakens — through management change, imitators, or simple drift — the building falls back into the general Region 15 pool, and the oversupply and ADR risks reprice accordingly. Verify the SEMARNAT authorisation is current alongside it.
...
## Resort-managed rental programme analysis

The managed programme takes 40–50% of gross — roughly double an independent Tulum manager — and allocates occupancy by rotation rather than by how well your specific unit performs. In exchange it supplies the resort's own booking channels and the brand that produces the ADR premium. Model both routes before assuming the programme is the default.
...
The investment case here is a bet on one demand segment continuing to grow, and it has three legs: a US wellness market expanding at an estimated 8–12% a year, on-property cenotes that beach condos cannot replicate, and retreat groups that book multiple units for several days at once. That last one is the most valuable and the least visible in occupancy averages.
