 ...
 *   3. фото меньше своего места: обложке нужен кадр 16:9 шириной от 960 точек (у низкой панорамы
 *      его ограничивает высота), фото в тексте 640.
 *
...
const small = photos.filter((p) => {
  ...
  // (1280x243 даёт кадр 432 точки, растянутый на всю ширину страницы).
  const coverW = Math.min(e.w, Math.round((e.h * 16) / 9));
  return (p.role === 'обложка' && coverW < 960) || (p.role === 'в тексте' && e.w < 640);
});
...
const show = (title, items, fmt) => {
  ...
  failed = true;
  console.log(`\n${title}: ${items.length}`);
  for (const x of items.slice(0, process.env.ALL ? 999 : 12)) console.log('  ' + fmt(x));
