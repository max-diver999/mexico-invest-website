...
## Adaptation to mexico-invest.com (2026-08-27)

Ported from capetown-invest-website at commit `90dc5e1`. Changes made here, each
verified against this repository's own labelled sets (bad = commit `5a8d980`,
the July 2026 "corpus lift"; good = the hand-written August waves; mid = cleaned
survivors of the July garbage):

- **Currency regexes** switched from rand to `MXN | US\$ | \$` in all five
  scoring locations plus `orphanFigures`. Verified: dollar amounts now appear in
  the unregistered-figures block of `--explain`.
- **Unit-type rules** rebuilt from this site's own garbage signature: the July
  lift stamped `"$1,500 benchmarks on <heading>"` (100+ instances) and
  `"$280,000 HOA proof"` / `"35% HOA proof"` (541 and dozens). The Cape Town
  word list (turnaround, LTV, occupancy…) appears nowhere in this corpus.
  Verified: fires across the bad set, zero hits in the good set and the current
  corpus ("SAT receipts, HOA proof" in a description has no leading figure and
  does not match).
- **Calibration on this corpus**: bad mean 0.0 (max 0), good mean 63.2 (min 58),
  mid 62.0, separation 63.2, 0/132 garbage files at or above the worst
  hand-written article. The old on-site rubric scored the same sets 87.6 / 90.5 /
  75.1 — separation 2.9 points, with honest prose ordered BELOW the garbage.
- Known weakness, recorded rather than hidden: good and mid sit close (63.2 vs
